...
public ledou_login_query(accountindex){	
	...
	if(login_accountindex < sizeof(structinfo)){
		format(httpquery,sizeof(httpquery),"qzapp.z.qq.com/qpet/cgi-bin/phonepk?cmd=index&zapp_sid=%s==&zapp_uin=%s",ledoupassword[login_accountindex],ledouaccount[login_accountindex]);
...
public ledou_caicai_query(accountindex){
	...
	if(caicai_accountindex < sizeof(structinfo)){
		format(httpquery,sizeof(httpquery),"qzapp.z.qq.com/qpet/cgi-bin/phonepk?zapp_uin=%s&zapp_sid=%s&cmd=caicai",ledouaccount[caicai_accountindex],ledoupassword[caicai_accountindex]);
		HTTP(0, HTTP_GET, httpquery,"", "ledou_query_result");
...
public ledou_eatcaicai_query(accountindex){
	...
	if(eatcaicai_accountindex < sizeof(structinfo)){	
		format(httpquery,sizeof(httpquery),"qzapp.z.qq.com/qpet/cgi-bin/phonepk?zapp_uin=%s&zapp_sid=%s&cmd=eatcaicai",ledouaccount[eatcaicai_accountindex],ledoupassword[eatcaicai_accountindex]);
		HTTP(0, HTTP_GET, httpquery,"", "ledou_query_result");
...
public ledou_fight_query(type,accountindex,clientindex){
	...
	if(type == 1 || type == 2){ //fight query
		if(ledouhttpqueryclient[clientindex][0] == '+'){ //gang ledou
			...
			format(httpquery,sizeof(httpquery),"qzapp.z.qq.com/qpet/cgi-bin/phonepk?zapp_uin=%s&zapp_sid=%s==&cmd=fight&B_UID=%s&page=1&type=2",ledouaccount[accountindex],ledoupassword[accountindex],ledouhttpqueryclient[clientindex]);
			...
			if(!strcmp(ledouhttpqueryclient[clientindex],"-1")){ //selfclient
				format(httpquery,sizeof(httpquery),"qzapp.z.qq.com/qpet/cgi-bin/phonepk?zapp_uin=%s&zapp_sid=%s==&cmd=fight&B_UID=%s&type=0&page=",ledouaccount[accountindex],ledoupassword[accountindex],ledouaccount[accountindex]);
				//printf("selffight:%s",httpquery);
			}
			else { //custom client
				format(httpquery,sizeof(httpquery),"qzapp.z.qq.com/qpet/cgi-bin/phonepk?zapp_uin=%s&zapp_sid=%s==&cmd=fight&B_UID=%s&type=0&page=",ledouaccount[accountindex],ledoupassword[accountindex],ledouhttpqueryclient[clientindex]);

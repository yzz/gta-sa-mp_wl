...
public ledou_fight_query(type,accountindex,clientindex){
	...
	if(type == 1 || type == 2){ //fight query
		if(ledouhttpqueryclient[clientindex][0] == '+'){ //gang ledou
			...
			format(httpquery,sizeof(httpquery),"qzapp.z.qq.com/qpet/cgi-bin/phonepk?zapp_uin=%s&zapp_sid=%s==&cmd=fight&B_UID=%s&type=1&page=",ledouaccount[accountindex],ledoupassword[accountindex],ledouhttpqueryclient[clientindex]);
